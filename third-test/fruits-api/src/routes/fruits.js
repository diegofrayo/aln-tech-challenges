...
router.get("/:fruitId", async function (req, res, next) {
  try {
...
router.delete("/:fruitId", async function (req, res, next) {
  try {
    const fruitToDelete = await firebaseService.get(
      `/fruits/${req.params.fruitId}`
    );

    if (!fruitToDelete) {
      res.status(404).json({
        error: `"${req.params.fruitId}" does not exist, so it wasn't deleted`,
      });

      return;
    }

    await firebaseService.remove(`/fruits/${req.params.fruitId}`);
...
router.put("/:fruitId", async function (req, res, next) {
  try {
    const fruitUpdates = req.body;
    const fruitToUpdate = await firebaseService.get(
      `/fruits/${req.params.fruitId}`
    );

    if (!fruitToUpdate) {
      res.status(404).json({
        error: `"${req.params.fruitId}" does not exist, so it wasn't updated`,
      });
      return;
    }

    await firebaseService.update(`/fruits/${req.params.fruitId}`, fruitUpdates);
